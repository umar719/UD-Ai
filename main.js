/* If You Copy, Don't Delete This Credit!!!
  Don`t Sell This Script Or I Take It Immediately
  Those who sell this script should just report/burn their accounts or beat up the person
  Move To Pairing Code
  For those who don't see the code, it's caused by the large number of plugins
  If You Want to Retrieve a Session, Backup All Plugins Files & Delete All Plugins Files
  Once you have finished, you can restore all the plugin files so they can be used
  Regards from U M A R ♡
*/
...
if(usePairingCode && !conn.authState.creds.registered) {
		...
		let phoneNumber = ''
		do {
			phoneNumber = await question(chalk.blueBright('Input a Valid number start with region code. Example : 923165123XXX:\n'))
		...
		setTimeout(async () => {
			...
			console.log(chalk.black(chalk.bgGreen(`Bbe Your Pairing Code : `)), chalk.black(chalk.white(code)))
		}, 3000)
	}
...
async function connectionUpdate(update) {
    ...
    if (connection == 'connecting') {
        console.log(chalk.redBright('⚡Activating UD Ai Bot, Please wait a moment...'));
    ...
    if (connection == 'close') {
        console.log(chalk.red('⏱️ connection lost & try to reconnect...'));
    }
...
global.reloadHandler = async function (restatConn) {
    ...
    try {
	// If you use replit, use sevenHoursLater and add it // pada const Handler
	// Default: server/vps/panel, replit + 7 clocks for pakistani watches
  ...
  conn.bye = '❖━━━━━━[ LEAVING ]━━━━━━❖\nDost @user 😻🥵'
  conn.spromote = '@user Now becomes admin!'
  conn.sdemote = '@user Now no longer an admin!'
  ...
  conn.sRestrictOn = 'Edit Group Info to change to admin only!'
...
async function _quickTest() {
    ...
    if (s.ffmpeg && !s.ffmpegWebp) {
        conn.logger.warn('Stickers May Not Animate without libwebp in ffmpeg (--enable-ibwebp while compiling ffmpeg)')
    }

    if (!s.convert && !s.magick && !s.gm) {
        conn.logger.warn('The Sticker Feature May Not Work Without imagemagick and libwebp in ffmpeg not installed (pkg install imagemagick)')
    }
...
setInterval(async () => {
  ...
  let bio = `Active Bots in time : ${uptime}`
