//changed the domain & changed some of it because it didn't work
	...
	if (chat && chat.updateAnime) {
		chat.updateAnimeInterval = setTimeout(async () => {
			try {
				...
				if (/Movie/.test(detailAnime.episode)) return conn.reply(m.chat, 'The bot cannot send the video file because it is too large...', quoted)
				let res = await downloadAnime(zippy?.['480p'] || zippy?.['720p'] || zippy?.['360p']).catch(() => null)
				if (!res) return conn.reply(m.chat, 'Download link not yet available...', quoted)
...
async function getDetailAnime(url) {
	...
	obj.title = $('div.pagetitle > h1').text().trim().replace(/Subtitle pakistan/, '')
